...
class CompressedQuaternionChannelChunk(Chunk):
    @staticmethod
    def parseData(binaryReader: Pure3DBinaryReader) -> list:
        ...
        for _ in range(numFrames):
            values.append(binaryReader.readPure3DCompressedQuaternion())
        
        return [version, param, frames, values]

    def __init__(
        self, 
        identifier: int = chunkIdentifiers.COMPRESSED_QUATERNION_CHANNEL, 
        children: list[Chunk] = None,
        version: int = 0,
        param: str = "",
        frames: list[int] = None,
        values: list[mathutils.Quaternion] = None,
    ) -> None:
    ...
    def writeData(self, binaryWriter: Pure3DBinaryWriter) -> None:
        ...
        for value in self.values:
            binaryWriter.writePure3DCompressedQuaternion(value)
